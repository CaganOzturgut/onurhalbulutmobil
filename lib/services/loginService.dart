...
class LoginService {
  var _url = Uri.http(OnurHalBulutAppSetting.TestUrl,'api/lisans/login');

  Future<LoginResponseDto> getLogin(LoginRequestDto2 login) async {

    var loginData = login;

    var aa = Crypt().encryptString(jsonEncode(login));

    var jsonData = null;
    var encrypted = aa;


    OnurHalBulutApp.userDto = login;
    await UserStorage().loginInfo(login);
    //print(aa);
    ...
    if (response.statusCode == 200) {
      ...
      LoginResponseDto kullanici =
      LoginResponseDto.fromJson(json.decode(gelen));

      UserStorage().writeUserInfo(kullanici);
      print(kullanici);
      return kullanici;
